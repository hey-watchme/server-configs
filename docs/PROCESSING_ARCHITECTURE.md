...
#### ⚡ 2025年11月5日更新：SKIP処理の統一化（特別扱いを廃止）
...
graph TB
    ...
    K -.->|必ず実行| SQS2[SQS: dashboard-summary<br/>累積分析開始]
...
**アーキテクチャの特徴（2025年11月5日改善）**:
- **トリガーLambda**: 1-2秒で完了（SQSにメッセージ送信のみ）
- **ワーカーLambda**: 必要な時間だけ実行（最大15分）
- **統一処理フロー**: SKIPステータスも通常処理と同じフローで実行
- **累積分析の保証**: 成功・失敗・SKIPに関わらず必ず累積分析をトリガー
...
- **2025年11月5日**:
  - **SKIP処理の統一化と06:00問題の根本解決**
  - SKIPステータスの特別扱いを廃止、全ケースで同じ処理フローを実行
  - 累積分析トリガーを必ず実行するように修正（成功・失敗・SKIPに関わらず）
  - Lambda audio-worker関数のcontinue文を削除し、処理フローを統一
  - SKIPを「単なるデータ欠損の一種」として扱う設計思想に変更
  - 夜間スキップ（23:00-05:59）後の06:00以降の処理が停止する問題を解決
- **2025年10月21日**:
