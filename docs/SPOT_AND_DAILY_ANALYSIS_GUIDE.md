...
WatchMe 分析設計ガイド - スポット分析 / デイリー分析のコンセプト定義
...
**役割**: スポット分析結果に重要度を付けて、不要なものを除外したり重要なものをピックアップしたりする

**評価スケール**: 0-5の整数

| Rating | 説明 | 使用目的 |
|--------|------|---------|
| **0** | 重要でない | 発話なし、背景音のみ → **自動除外対象** |
| **1** | 低い重要度 | 断片的な会話、日常的な背景音 |
| **2** | やや重要 | 短い会話、ルーティン活動 |
| **3** | 普通 | 明確な会話内容、一般的な活動 |
| **4** | 重要 | 興味深い会話、記憶に残る出来事 |
| **5** | 非常に重要 | 印象的な出来事、顕著な感情表現 |

**評価方法**:
- LLMが`vibe_transcriber_result`の内容を基に自動評価
- 「発話なし」が含まれる場合 → rating = 0（自動判定）
- それ以外 → 会話内容・感情・活動の重要度に基づいて1-5を評価

**使い道**:
- ダッシュボードで「rating ≥ 3のみ表示」などのフィルタリング
- 1日に何十件も発生するスポット分析から、見るべきものを自動選別
- 無音・無発話の録音を自動除外し、ノイズを削減
