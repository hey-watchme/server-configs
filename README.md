...
| 分析 | 概要 |
|------|------|
| **ASR**（Vibe Transcriber） | 音声の文字起こし |
| **SED**（Behavior Features） | 527 種類の音響イベント検出 |
| **SER**（Emotion Features） | 8 感情の認識 |

この 3 つの結果をプロンプトに集約（Aggregator API）し、最新の LLM で分析します。

| 分析レベル | 説明 |
|-----------|------|
| **Spot 分析** | 1 録音単位の分析。ASR/SED/SER をもとに LLM が心理的プロファイルを生成 |
| **Daily 分析** | その日の Spot 分析を統合して 1 日の傾向を分析 |
| **Weekly 分析** | Daily 分析を統合して週単位の変化・傾向を分析 |

### 提供形態

- **コンシューマー向け**: iPhone アプリで個人のメンタルヘルスを継続モニタリング
- **B2B 向け（WatchMe Business）**: 障害児通所支援事業所向けのバーティカル AI BPO。支援員の記録・観察業務を AI が補完し、個別支援計画の作成を効率化する業務改善ソリューション
